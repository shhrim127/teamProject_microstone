...
import { getQuizList,addToWrongAnswerNote } from '../../api/GetQuiz';
...
const QuizSolvePage = () => {
  ...
  useEffect(() => {
    const fetchQuizData = async () => {
      try {
        const response = await getQuizList(currentPage, setId);
  ...
  const handleAddToWrongAnswerNote = async () => {
    const result = await Swal.fire({
      title: '추가 확인',
      text: '이것도 오답노트에 추가하시겠습니까?',
      icon: 'question',
    ...
    if (result.isConfirmed) {
      try {
        const response = await addToWrongAnswerNote(questions[currentQuestionIndex].id);
        ...
        if (response.status === 200) {
          ...
          await Swal.fire({
            title: '경고!',
            text: '오답노트에 이미 등록된 문제입니다!',
            icon: 'warning',
          });
        }
      } catch (error) {
        console.error('오답노트 추가 중 오류 발생:', error);
        await Swal.fire({
          title: '실패!',
          text: '오답노트 추가에 실패했습니다.',
          icon: 'error',
        });
      }
    } else {
                      ...
                      <div className="chat-history-header border-bottom">
                        <div className="d-flex align-items-center">
                          <button
                            ...
                            style={{ height: 42 }}
                            onClick={() => handleAddToWrongAnswerNote()}
                          >
                            오답노트 추가
                          </button>
                        </div>
